...
export default class GroupAddMemberView extends BaseComponent {
    ...
    requestSearchPlayer(text) {
        ...
        let url = this.getConfig().getBaseUrl() + ApiService.group_search_user(this.groupId, text, this.page);
        console.log("url = ", url);
    ...
    onSearchPlayerResponse(jsonData) {
        ...
        if (this.model.getErrorCode() === 0) {
            if (this.model.getListFriendData().length > 0) {
                ...
                //  let listData = this.model.getListFriendData();
                // this.Logger().log('....................................... page search ', this.isFocus);
                // let listData = this.page === 1 ? this.model.getListFriendData() : [...this.state.dataSource, ...checkList];
                // let listData = this.isFocus ? checkList : [...this.state.dataSource, ...checkList];
    ...
    onRequestInvites() {
        this.internalLoading.showLoading();
        let url = this.getConfig().getBaseUrl() + ApiService.group_add_member(this.groupId);
    ...
    setCallBack() {
        ...
        if (params.AddMemberCallback) {
            params.AddMemberCallback();
        }
        if (params.refresh) {
            params.refresh();
        }
