...
export default class HomeView extends BaseComponent {
    ...
    constructor(props) {
        ...
        this.props.navigation.addListener('didFocus', this.onScreenDidFocus);
        // this.props.navigation.addListener('didBlur', this._handleDidBlur.bind(this));
        this.navigation.addListener('didFocus', this.onScreenDidFocus);
    }

    onScreenDidFocus = state => {
        console.log('HomeViewsssssssssss._onNavigatorEvent');
        // this.setChange();
        if (this.refUserHomeView)
            this.refUserHomeView.updateUserProfile();
    };

    // _handleStateChange2 = state => {
    //     console.log('HomeViewsssssssssss._handleStateChange2');
    //     // this.setChange();
    // };

    // _handleDidBlur() {
    //     console.log('HomeViewsssssssssss._handleDidBlur');
    //     // this.setChange();
    // }
        ...
        return (
            <View style={styles.container} >
                ...
                <UserHomeView
                    ...
                    onSearchEverythingPress={this.onSearchEverythingPress} />
    ...
    scrollCallback(event) {
        ...
        if (isActionButtonVisible !== this.createFlightShowing) {
            this.createFlightShowing = isActionButtonVisible;
            if (this.btnChatInHome) {
                this.btnChatInHome.setVisible(this.createFlightShowing);
            }
        }
    ...
    */
    ...
    onSearchEverythingPress() {
        ...
        if (this.navigation) {
            this.navigation.navigate('search_everything_view');
        }
    }
    ...
    onAvatarClick() {
        ...
        if (this.navigation != null) {
            this.navigation.navigate('persional_information', { "puid": this.uid, personalCallback: this.onPersonalCallback.bind(this) });
        }
    }

    onPersonalCallback() {
        console.log('onPersonalCallback');
        this.refUserHomeView.updateUserProfile()
    }
