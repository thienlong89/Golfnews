...
export default class ComPersonalImages extends BaseComponent {
    ...
    onPressImage(data, { isLastImage, index }) {
        console.log('onPressImage', data, isLastImage);
        let navigation = StaticProps.getAppSceneNavigator();
        if (navigation) {
            // if (isLastImage) {
            //     navigation.navigate('image_comment_flight_view')
            // } else {
            navigation.navigate('show_image_slide_view', {
                'dataList': this.state.imgs,
                'index': index,
            })
            // }
    ...
    onAddMoreImagePress() {
        let navigation = StaticProps.getAppSceneNavigator();
        if (navigation) {
            navigation.navigate('flight_list_add_image_view',
                {
                    addImageCallback: this.onAddImageCallback.bind(this)
                })
        }
    }
