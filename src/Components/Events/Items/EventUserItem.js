...
export default class EventUserItem extends BaseComponent {
    constructor(props) {
        super(props);
        this.state = {
            isLoading: false,
    ...
    onItemPressAndHole(){
        ...
        if((this.getAppUtil().replaceUser(data.getId()) === this.getAppUtil().replaceUser(this.getUserInfo().getId())) || (!isHost)){
            return;
        }
        this.setState({
            showPopup: true
    ...
    onRemoveUserClick(){
        let{data,event_name} = this.props;
        //console.log("event remove user data : ",data);
        //this.sendRequestRemoveUser();
        let msg = this.t('event_remove_user').format(data.fullname,event_name);
        this.popupComfirmView.okCallback = this.sendRequestRemoveUser.bind(this);
        this.popupComfirmView.cancelCallback = this.cancelRemoveUser.bind(this);
        this.popupComfirmView.setMsg(msg);
    }
    ...
    sendRequestRemoveUser(){
        let{data,removeFromListView,event_id} = this.props;
        console.log("event remove user data : ",data);
        let url = this.getConfig().getBaseUrl() + ApiService.event_remove_member(event_id,data.id);
        console.log('url xoa user event : ',url);
        ...
        Networking.httpRequestGet(url,(jsonData)=>{
            ...
            console.log("xoa user khoi event : ",jsonData);
        ...
        return (
            <TouchableWithoutFeedback onPress={() => { this.setState({ showPopup: false }) }}>
                <View style={{ alignItems: 'center', justifyContent: 'center', marginTop: showPopup ? verticalScale(5) : 0 }}>
                    <PopupComfirmView ref={(popupComfirmView)=>{this.popupComfirmView = popupComfirmView;}}/>
                    ...
                    <Touchable onPress={this.onItemClick.bind(this)} onLongPress={this.onItemPressAndHole.bind(this)}>
                        <View style={[styles.container,{ width: screenWidth, borderTopColor: showPopup ? '#e3e3e3' : null, borderTopWidth: showPopup ? verticalScale(1) : 0 }]}>
                            {this.renderLoading()}
