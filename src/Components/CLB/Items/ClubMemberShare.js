...
export default class ClubMemberShare extends BaseComponent {
    constructor(props) {
        super(props);
        this.state = {
            "is_friend": true,
    ...
    componentDidMount() {
        // this.setState({
        //     is_friend: this.props.data.is_friend,
        //     is_waiting_friend_request: this.props.data.is_waiting_friend_request
        // });
    }
    ...
    render() {
        ...
        let{data} = this.props;
        //let{isLoading,is_waiting_friend_request} = this.state;
        return (
            <View style={styles.container}>
                ...
                {/* <MyView style={styles.add_friend_view} hide={this.checkMe(data.userId)}>
                    <Touchable onPress={this.onItemClick.bind(this)}>
                        <MyView style={[styles.add_friend_button, { borderColor: this.getColorText(), backgroundColor: this.getColorView() }]} hide={isLoading}>
                            <Text allowFontScaling={global.isScaleFont} style={{ textAlign: 'center', color: this.getColorText() }}>{this.getStateFriend(this.state.is_friend, is_waiting_friend_request)}</Text>
                        </MyView>
                    </Touchable>
                    <ItemLoading ref={(itemLoading)=>{this.itemLoading = itemLoading;}} top={20} right={20}/>
                </MyView>
                <MyView style={styles.add_friend_view} hide={!this.checkMe(data.userId)}>
                </MyView> */}
            </View>
