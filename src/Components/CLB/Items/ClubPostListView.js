...
export default class ClubPostListView extends BaseComponent {
        ...
        return (
            <View style={styles.container}>
                ...
                {/* <PopupMenuPostInfo
                    ...
                    onPostMenuItemPress={this.onPostMenuItemPress.bind(this)} /> */}
                <PopupMenuPostInfo
                    ref={(refPopupMenuPostInfo) => { this.refPopupMenuPostInfo = refPopupMenuPostInfo }}
    ...
    requestTopicClub() {
        ...
        let self = this;
        let url = this.getConfig().getBaseUrl() + ApiService.get_all_post(this.page);
        console.log('url', url);
    ...
    onPostMenuItemPress(menuItem) {
        console.log('onPostMenuItemPress', menuItem)
    }
    ...
    onPostCallback(post) {
        console.log('onPostCallback', post);
        this.setState({
            isPosting: true
        }, () => {
            if (post.imgs.length > 0) {
                ...
                this.requestCreatePost(post.club.id, post.topic, post.content, []);
            }
        });
    }
    ...
    onUploadSuccess(post, jsonData) {
        ...
        if (jsonData.error_code === 0) {
            ...
            this.requestCreatePost(post.club.id, post.topic, post.content, uris);
        }
